...
import { ArrowRight, Check, Compass, Flag, Globe2, MapPin, Moon, MousePointer2, RotateCcw, Search, Shuffle, Sun, X, ZoomIn, ZoomOut } from 'lucide-react';
...
function App() {
  const [memorized, setMemorized] = useState(() => saved('atlas-memorized-v1', []));
  ...
  useEffect(() => { localStorage.setItem('atlas-memorized-v1', JSON.stringify(memorized)); }, [memorized]);
  ...
  const toggleMemorized = () => setMemorized(prev => prev.includes(selected) ? prev.filter(id => id !== selected) : [...prev,selected]);
