...
export const countries = shapes.map(shape => {
  ...
  return { id: code, name: names[code] || countryInfo[code]?.name || record.country, capital: capitals[code] || countryInfo[code]?.capital || record.capital || '—', continent: record.continent, shape: code === 'BH' ? bahrainDetail : code === 'PS' ? { ...shape, geometry: palestineArea } : shape };
}).filter(Boolean);
